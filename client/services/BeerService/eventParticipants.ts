...
export interface EventParticipantsService {
  ...
  newParticipant: (eventId: string, participantData: { name: string; email: string }) => Promise<{ success: boolean }>;
}
...
export const eventParticipants: EventParticipantsService = {
  ...
  async newParticipant(eventId: string, participantData: { name: string; email: string }): Promise<{ success: boolean }> {
