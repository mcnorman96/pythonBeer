...
describe('authService', () => {
  ...
  describe('registerUser', () => {
    it('throws error if required fields are missing', async () => {
      await expect(authService.registerUser({ username: '', password: '', email: '' })).rejects.toThrow('All fields are required');
    });
  ...
  describe('login', () => {
    it('throws error if required fields are missing', async () => {
      await expect(authService.login({ username: '', password: '' })).rejects.toThrow('All fields are required');
    });
