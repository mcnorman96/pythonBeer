...
def test_new_beer_success(client, mocker):
    ...
    response = client.post('/beer/new', json={
        'name': 'Test Beer',
...
def test_all_beers(client, mocker):
    ...
    mocker.patch('services.beer_services.BeerService.get_all', return_value=[mock_beer1, mock_beer2])
...
def test_all_beers_error(client, mocker):
    mocker.patch('services.beer_services.BeerService.get_all', side_effect=ValueError('DB error'))
...
def test_search_beers(client, mocker):
    ...
    mocker.patch('services.beer_services.BeerService.search_by_name', return_value=mock_beer_list)
...
def test_search_beers_error(client, mocker):
    mocker.patch('services.beer_services.BeerService.search_by_name', side_effect=ValueError('Search error'))
