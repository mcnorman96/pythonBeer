...
def test_new_event_participant_success(client, mocker):
    ...
    mocker.patch(
        "services.event_participant_service.EventParticipantService.create",
        return_value=mock_event_participant,
    )
...
def test_new_event_participant_unauthorized(client, mocker):
    mocker.patch(
        "routes.event_participant.get_valid_user_id", side_effect=Unauthorized()
    )
    response = client.post("/events/2/participants/new")
...
def test_all_event_participant_success(client, mocker):
    ...
    mocker.patch(
        "services.event_participant_service.EventParticipantService.get_all_users_in_event",
        return_value=mock_event_participants,
    )
