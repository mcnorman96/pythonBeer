...
def test_new_rating_success(client, mocker):
    ...
    mocker.patch(
        "services.ratings_service.RatingsService.create", return_value=mock_rating
    )
...
def test_get_rating_by_id_success(client, mocker):
    ...
    mocker.patch(
        "services.ratings_service.RatingsService.getRating", return_value=mock_rating
    )
    mocker.patch("routes.ratings.get_valid_user_id", return_value=4)
    response = client.get(
        "/ratings/getRating", query_string={"event_id": 1, "beer_id": 2}
    )
...
def test_get_all_ratings_for_beer_success(client, mocker):
    ...
    mocker.patch(
        "services.ratings_service.RatingsService.getAllRatingsForBeer",
...
def test_get_all_ratings_for_beer_no_ratings(client, mocker):
    mocker.patch(
        "services.ratings_service.RatingsService.getAllRatingsForBeer", return_value=[]
    )
    response = client.get("/ratings/all", query_string={"beer_id": 3, "event_id": 2})
...
def test_toplist_success(client, mocker):
    ...
    mocker.patch(
        "services.ratings_service.RatingsService.get_toplist", return_value=mock_toplist
    )
...
def test_toplist_error(client, mocker):
    mocker.patch(
        "services.ratings_service.RatingsService.get_toplist",
        side_effect=ValueError("DB error"),
    )
...
def test_toplist_by_event_id_success(client, mocker):
    ...
    mocker.patch(
        "services.ratings_service.RatingsService.get_toplist_by_event",
        return_value=mock_toplist,
    )
...
def test_toplist_by_event_id_no_ratings(client, mocker):
    mocker.patch(
        "services.ratings_service.RatingsService.get_toplist_by_event", return_value=[]
    )
